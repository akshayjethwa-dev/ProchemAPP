...
const BuyerHome: React.FC<Props> = ({ products, onViewProduct, onViewCart, onViewHelp, onTrackOrder, cartCount }) => {
    ...
    <div className="flex-1 flex flex-col bg-gray-50 overflow-y-auto hide-scrollbar pb-24">
      ...
      <div className="bg-[#004AAD] pt-12 pb-6 px-6 sticky top-0 z-20 shadow-lg">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center space-x-4">
            <button onClick={onViewHelp} className="text-white p-1">
            ...
            <button onClick={onViewCart} className="relative text-white p-1">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
              {cartCount > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center border-2 border-[#004AAD]">{cartCount}</span>}
            </button>
          </div>
        </div>
        <div className="relative">
          <input 
            ...
            className="w-full bg-white p-3.5 pl-10 rounded-xl text-sm focus:outline-none"
          />
      ...
      <div className="p-6">
        ...
        <div className="flex overflow-x-auto space-x-3 hide-scrollbar pb-2">
          <button 
            ...
            className={`px-6 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${selectedCat === 'All' ? 'bg-green-600 text-white' : 'bg-white text-gray-600 border border-gray-200'}`}
          >
            All
          </button>
          {CATEGORIES.map(cat => (
            <button 
              ...
              className={`px-6 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${selectedCat === cat ? 'bg-green-600 text-white' : 'bg-white text-gray-600 border border-gray-200'}`}
            >
              {cat}
            </button>
          ))}
      ...
      <div className="px-6 space-y-4">
        <div className="flex items-center justify-between">
           ...
           <button className="text-[#004AAD] text-xs font-bold">See All</button>
        </div>
        ...
        {filtered.map(p => (
          ...
          >
            <img src={p.image} className="w-28 h-28 object-cover" alt={p.name} />
            <div className="p-4 flex-1 flex flex-col justify-between">
              <div>
                <h3 className="font-bold text-sm text-gray-900 leading-tight">{p.name}</h3>
                <p className="text-[10px] text-gray-500 mt-1">{p.sellerName} • ⭐ {p.sellerRating}</p>
              </div>
              <div className="flex items-end justify-between mt-2">
                <div>
                  <p className="text-[#004AAD] font-bold">₹{p.pricePerUnit.toLocaleString()}</p>
                  <p className="text-[10px] text-gray-400">per {p.unit} + GST</p>
                </div>
                <div className="bg-gray-50 px-2 py-1 rounded-lg border border-gray-100">
                  <p className="text-[10px] text-gray-600 font-bold">MOQ: {p.moq}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="mt-8 px-6 grid grid-cols-2 gap-4">
         <div onClick={onTrackOrder} className="bg-blue-50 p-4 rounded-2xl border border-blue-100 cursor-pointer">
            <span className="text-2xl">📦</span>
            <h4 className="font-bold text-blue-900 mt-2">Active Orders</h4>
            <p className="text-[10px] text-blue-700">Track shipments</p>
         </div>
         <div className="bg-green-50 p-4 rounded-2xl border border-green-100">
            <span className="text-2xl">📑</span>
            <h4 className="font-bold text-green-900 mt-2">GST Invoices</h4>
            <p className="text-[10px] text-green-700">Download reports</p>
         </div>
      </div>

      {/* Bottom Nav Simulation */}
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white border-t border-gray-100 flex justify-around py-3 px-6 z-30">
        <button className="flex flex-col items-center space-y-1 text-[#004AAD]">
           <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M10,20V14H14V20H19V12H22L12,3L2,12H5V20H10Z" /></svg>
